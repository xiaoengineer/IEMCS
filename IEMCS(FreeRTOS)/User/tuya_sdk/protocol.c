...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_SMOKE_CONCENTRATION, DP_TYPE_VALUE},
  {DPID_LIGHT, DP_TYPE_BOOL},
  {DPID_FAN, DP_TYPE_BOOL},
  {DPID_HEATER, DP_TYPE_BOOL},
  {DPID_L, DP_TYPE_VALUE},
  {DPID_SMLAL, DP_TYPE_ENUM},
  {DPID_LIGHT_A, DP_TYPE_BOOL},
  {DPID_TEMP_A, DP_TYPE_BOOL},
  {DPID_HUMI_A, DP_TYPE_BOOL},
  {DPID_TEMP_S, DP_TYPE_VALUE},
  {DPID_HUMI_S, DP_TYPE_VALUE},
};
...
//    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,当前湿度周期上报); //VALUE型数据上报;
   ...
   mcu_dp_enum_update(DPID_SMLAL,); //枚举型数据上报;当前烟雾报警


    */
	 //mcu_dp_enum_update(DPID_TEMP_ALARM,2); //枚举型数据上报;
	mcu_dp_value_update(DPID_TEMP_CURRENT,sensor_data.Temperature*10); //VALUE型数据上报;当前当前温度
	mcu_dp_value_update(DPID_HUMIDITY_VALUE,sensor_data.Humidity); //VALUE型数据上报;当前当前湿度
	mcu_dp_value_update(DPID_L,sensor_data.lux); //VALUE型数据上报;当前亮度值
	mcu_dp_value_update(DPID_SMOKE_CONCENTRATION,sensor_data.MQ2); //VALUE型数据上报;当前烟雾浓度
	
    mcu_dp_value_update(DPID_MAXTEMP_SET,limit_data.Tempe_max); //VALUE型数据上报;当前温度上限设置
    mcu_dp_value_update(DPID_MINITEMP_SET,limit_data.Tempe_min); //VALUE型数据上报;当前温度下限设置
    mcu_dp_value_update(DPID_MAXHUM_SET,limit_data.Humi_max); //VALUE型数据上报;当前湿度上限设置
    mcu_dp_value_update(DPID_MINIHUM_SET,limit_data.Humi_min); //VALUE型数据上报;	当前湿度下限设置
	
	
	mcu_dp_bool_update(DPID_LIGHT_A,auto_set_flag.light_a); //BOOL型数据上报;当前灯光自动
    mcu_dp_bool_update(DPID_TEMP_A,auto_set_flag.temp_a); //BOOL型数据上报;当前温度自动
    mcu_dp_bool_update(DPID_HUMI_A,auto_set_flag.hmui_a); //BOOL型数据上报;当前湿度自动
	
	mcu_dp_value_update(DPID_TEMP_S,set_data.temp_v); //VALUE型数据上报;当前温度设置
    mcu_dp_value_update(DPID_HUMI_S,set_data.humi_v); //VALUE型数据上报;当前湿度设置
}
...
static unsigned char dp_download_maxtemp_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    limit_data.Tempe_max = maxtemp_set;
	buff[0] = limit_data.Tempe_max;
	I2C_EE_ByteWrite(buff,0x20);
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_minitemp_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    limit_data.Tempe_min = minitemp_set;
	buff[0] = limit_data.Tempe_min;
	I2C_EE_ByteWrite(buff,0x21);
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_maxhum_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    limit_data.Humi_max = maxhum_set;
	buff[0] = limit_data.Humi_max;
	I2C_EE_ByteWrite(buff,0x22);	
...
static unsigned char dp_download_minihum_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    limit_data.Humi_min = minihum_set;
	
	buff[0] = limit_data.Humi_min;
	I2C_EE_ByteWrite(buff,0x23);
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_bright_value_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    set_data.light_v = bright_value;
	buff[0] =( set_data.light_v &0xff00) >> 8;
	buff[1] = set_data.light_v &0x00ff;
	I2C_EE_PageWrite(buff,0x24,2);
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_fan_handle(const unsigned char value[], unsigned short length)
{
    ...
    fan = mcu_get_dp_download_bool(value,length);
    if(fan == 0) {
        //开关关
		set_data.fan_b = 0;
    }else {
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_FAN,fan);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_heater_handle
功能描述 : 针对DPID_HEATER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_heater_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    heater = mcu_get_dp_download_bool(value,length);
    if(heater == 0) {
        //开关关
		set_data.heater_b = 0;
    }else {
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_HEATER,heater);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_light_a_handle
功能描述 : 针对DPID_LIGHT_A的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_light_a_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    light_a = mcu_get_dp_download_bool(value,length);
    if(light_a == 0) {
        //开关关
		auto_set_flag.light_a = 0;
    }else {
        //开关开
		auto_set_flag.light_a = 1;
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LIGHT_A,light_a);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_temp_a_handle
功能描述 : 针对DPID_TEMP_A的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_temp_a_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    temp_a = mcu_get_dp_download_bool(value,length);
    if(temp_a == 0) {
        //开关关
		auto_set_flag.temp_a = 0;
    }else {
        //开关开
		auto_set_flag.temp_a = 1;
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_TEMP_A,temp_a);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_humi_a_handle
功能描述 : 针对DPID_HUMI_A的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_humi_a_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char humi_a;
    
    humi_a = mcu_get_dp_download_bool(value,length);
    if(humi_a == 0) {
        //开关关
		auto_set_flag.hmui_a = 0;
    }else {
        //开关开
		auto_set_flag.hmui_a = 1;
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_HUMI_A,humi_a);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_temp_s_handle
功能描述 : 针对DPID_TEMP_S的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_temp_s_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long temp_s;
    uint8_t buff[1] = {0};
    temp_s = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    set_data.temp_v = temp_s;
	buff[0] = set_data.temp_v;
	I2C_EE_ByteWrite(buff,0x26);
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_TEMP_S,temp_s);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_humi_s_handle
功能描述 : 针对DPID_HUMI_S的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_humi_s_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long humi_s;
    uint8_t buff[1] = {0};
    humi_s = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    
    */
    set_data.humi_v = humi_s;
	buff[0] = set_data.humi_v;
	I2C_EE_ByteWrite(buff,0x27);	
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_HUMI_S,humi_s);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_HEATER:
            //加热器处理函数
            ret = dp_download_heater_handle(value,length);
        break;
        case DPID_LIGHT_A:
            //灯光自动处理函数
            ret = dp_download_light_a_handle(value,length);
        break;
        case DPID_TEMP_A:
            //温度自动处理函数
            ret = dp_download_temp_a_handle(value,length);
        break;
        case DPID_HUMI_A:
            //湿度自动处理函数
            ret = dp_download_humi_a_handle(value,length);
        break;
        case DPID_TEMP_S:
            //温度设置处理函数
            ret = dp_download_temp_s_handle(value,length);
        break;
        case DPID_HUMI_S:
            //湿度设置处理函数
            ret = dp_download_humi_s_handle(value,length);
        break;
...
void mcu_write_rtctime(unsigned char time[])
{
	...
	DateToUpdate.Year = time[1];
	DateToUpdate.Month = time[2];
	DateToUpdate.Date =time[3];
	DateToUpdate.WeekDay = time[7];
