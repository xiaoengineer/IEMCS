...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_LIGHT_INTENSITY, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_SMOKE_CONCENTRATION,当前烟雾浓度); //VALUE型数据上报;*/
    mcu_dp_value_update(DPID_LIGHT_INTENSITY,sensor_data.lux); //当前光照强度
	mcu_dp_value_update(DPID_HUMIDITY_VALUE,sensor_data.Humidity); //当前湿度;
	mcu_dp_value_update(DPID_TEMP_CURRENT,sensor_data.Temperature *10); //当前温度上报;
	mcu_dp_value_update(DPID_SMOKE_CONCENTRATION,sensor_data.MQ2); //当前烟雾浓度上报;*/
